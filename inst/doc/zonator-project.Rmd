...
single function `create_zproject()` that can be used to either

1. Create a new Zonation project, or
1. Create zonator project based on existing data

In both cases, `zonator` works by parsing real input files on your file system 
(needed by Zonation) into R data structures. Formally, `create_zproject()` 
...
## 2. Create a new zonator project
...
**NOTE!** For the time being, you will have to manually delete the project on
file system if you want to recreate it. Current behavior is such, that creating
the project files will be followed by loading those same files (see below).

You can gove extra arguments to `create_zproject()` that will be passed on to
...
can be parsed into R data structures. The tutorial data include results of the 
