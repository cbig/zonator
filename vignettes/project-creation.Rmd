...
```{r opendir}
opendir(test.project)
```

Individual variants (i.e. runs) can be extracted form the project using an 
index number. `navariants()` will tell how many variants are included in the 
project.

```{r extract-variant}
nvariants(test.project)
variant.1 <- get_variant(test.project, 1)
```
...
```{r variants-by-name}
names(test.project)
variant.1 <- get_variant(test.project, "01_core_area_zonation")
```