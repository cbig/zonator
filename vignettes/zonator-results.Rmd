...
Naturally, all mehtods defined for a [RasterStack](http://hosho.ees.hokudai.ac.jp/~kubo/Rdoc/library/raster/html/stack.html) object are available. For example, checking 
...
Masking is another usefuls analysis tool. In the following example we are using
the maskfile `mask_rs.tif` shipping with Zonation basic tutorial in order to
...
package for plottign raster data. For example, `levelplot()` can be used to 
